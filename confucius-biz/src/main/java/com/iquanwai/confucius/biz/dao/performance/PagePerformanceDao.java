...
public class PagePerformanceDao extends DBUtil {
    ...
    public List<PagePerformance> queryAboutAddTime(String url,String startTimeStr, String endTimeStr){
        QueryRunner run = new QueryRunner(getDataSource());
        ResultSetHandler<List<PagePerformance>> h = new BeanListHandler<>(PagePerformance.class);
        String sql = "SELECT * FROM PagePerformance where Url = ? and AddTime >= ? and AddTime <= ? order by AddTime";
        try {
            return run.query(sql, h, url, startTimeStr, endTimeStr);
        } catch (SQLException e) {
            logger.error(e.getLocalizedMessage(), e);
        }
